...
func TestCatalogPriceValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing Product tax details", mutate: func(price *stripe.Price) { price.Product.TaxDetails = nil }},
		{name: "wrong Product tax code", mutate: func(price *stripe.Price) { price.Product.TaxDetails.TaxCode = "txcd_10000000" }},
...
func testCatalogPrice(configured CatalogPrice, amount int64, recurring bool) *stripe.Price {
	price := &stripe.Price{
		...
		Product: &stripe.Product{
			...
			Object:     "product",
			TaxDetails: &stripe.ProductTaxDetails{TaxCode: glassEQTaxCode},
		},
