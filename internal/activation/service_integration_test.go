...
func TestActivationCleanupIsBoundedWithPostgreSQL(t *testing.T) {
	...
	for index, expiresAt := range []time.Time{now.Add(-2 * time.Hour), now.Add(-time.Hour), now.Add(24 * time.Hour)} {
		...
		_, err = database.ExecContext(context.Background(), `
			...
			) VALUES ($1, 'lic_cleanup', $2, $3, $4, $3)`,
			fmt.Sprintf("red_cleanup_%d", index), []byte("ciphertext"),
			expiresAt.Add(-recoveryTokenLifetime), expiresAt)
		if err != nil {
			t.Fatalf("insert recovery email %d: %v", index, err)
		}
	}
