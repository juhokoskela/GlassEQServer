...
func TestBillingConfiguration(t *testing.T) {
	...
	got, err = load(mapLookup(values))
	if err != nil || got.Billing == nil || got.Billing.AccountID != "123456789012" || got.Billing.PerpetualProductID != "prod_perpetual" {
		t.Fatalf("billing configuration: %+v, %v", got.Billing, err)
	}
	for _, key := range []string{"GLASSEQ_STRIPE_SECRET_KEY", "GLASSEQ_STRIPE_PERPETUAL_PRODUCT_ID", "GLASSEQ_STRIPE_EVENT_SOURCE", "GLASSEQ_BILLING_QUEUE_URL"} {
